...
impl Simulation {
    ...
    for z in 0..self.nz {
      ...
      for y in 0..self.ny {
        ...
        for x in 0..self.nx {
          ...
          let u_lap =
            self.u[idx(self.nx, self.ny, xm, y, z)] + self.u[idx(self.nx, self.ny, xp, y, z)]
              + self.u[idx(self.nx, self.ny, x, ym, z)] + self.u[idx(self.nx, self.ny, x, yp, z)]
              + self.u[idx(self.nx, self.ny, x, y, zm)] + self.u[idx(self.nx, self.ny, x, y, zp)]
              - 6.0 * u;

          let v_lap =
            self.v[idx(self.nx, self.ny, xm, y, z)] + self.v[idx(self.nx, self.ny, xp, y, z)]
              + self.v[idx(self.nx, self.ny, x, ym, z)] + self.v[idx(self.nx, self.ny, x, yp, z)]
              + self.v[idx(self.nx, self.ny, x, y, zm)] + self.v[idx(self.nx, self.ny, x, y, zp)]
              - 6.0 * v;
          ...
          self.update_chunk_ranges_for_point(x, y, z, v_next);
        }
      }
    }
    ...
    for cz in min_cz..=max_cz {
      for cy in min_cy..=max_cy {
        for cx in min_cx..=max_cx {
          ...
          // Sphere-AABB cull.
          let d2 = axis_dist2(cam_x, wx0, wx1) + axis_dist2(cam_y, wy0, wy1) + axis_dist2(cam_z, wz0, wz1);
          if d2 > rad2 {
            continue;
          }
          ...
          let cyw = (wy0 + wy1) * 0.5;
          let czw = (wz0 + wz1) * 0.5;
          let dist = ((cxw - cam_x) * (cxw - cam_x) + (cyw - cam_y) * (cyw - cam_y) + (czw - cam_z) * (czw - cam_z)).sqrt();
          ...
          mesh_region_append(&self.v, self.nx, self.ny, self.nz, iso, color, &mut self.mesh, x0, x1, y0, y1, z0, z1, stride);
        }
      }
    }
  }
  ...
  pub fn generate_isosurface_mesh(&mut self, iso: f32, r: f32, g: f32, b: f32, a: f32) {
    generate_isosurface_mesh(&self.v, self.nx, self.ny, self.nz, iso, [r, g, b, a], &mut self.mesh);
  }
